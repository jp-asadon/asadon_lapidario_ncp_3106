...
// function validateForm() {
//   let isValid = true;
//   const errorMessage = "Kindly answer all parts of the form.";
//   const tabs = document.getElementsByClassName("tab");
//   const inputs = tabs[currentTab].querySelectorAll("input, select");

//   // Loop through all inputs in the current tab
//   inputs.forEach((input) => {
//     const errorSpan = input.nextElementSibling; // Error message span assumed right after the input
...
function nextPrev(n) {
  ...
  // Submit form if it's the last tab
  if (currentTab >= tabs.length) {
    document.getElementById("regForm").submit();
    return false;
  }
...
function validateForm() {
  ...
  if (college === "") {
    ...
    collegeErr.textContent = "Please enter a valid College name in uppercase. (e.g., CENGG, CAS, CCSS)";
    isValid = false;
  } else {
    collegeErr.textContent = "";
  }
